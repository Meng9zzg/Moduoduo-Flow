...
const APIKeyDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
    ...
    const addNewKey = async () => {
        try {
            ...
            if (createResp.data) {
                enqueueSnackbar({
                    message: t('dialog:apiKey.addSuccess'),
                    options: {
            ...
            enqueueSnackbar({
                message: t('dialog:apiKey.addError', {
    ...
    const saveKey = async () => {
        try {
            ...
            if (saveResp.data) {
                enqueueSnackbar({
                    message: t('dialog:apiKey.saveSuccess'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            if (setError) setError(error)
            enqueueSnackbar({
                message: t('dialog:apiKey.saveError', {
                    error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                }),
                options: {
                    key: new Date().getTime() + Math.random(),
        ...
        >
            ...
            <DialogContent>
                {dialogProps.type === 'EDIT' && (
                    <Box sx={{ p: 2 }}>
                        <Typography variant='overline'>{t('dialog:apiKey.apiKeyLabel')}</Typography>
                        <Stack direction='row' sx={{ mb: 1 }}>
                            ...
                            <IconButton
                                title={t('dialog:apiKey.copyTooltip')}
                                color='success'
                                onClick={(event) => {
                ...
                <Box sx={{ p: 2 }}>
                    <Stack sx={{ position: 'relative' }} direction='row'>
                        <Typography variant='overline'>{t('dialog:apiKey.keyNameLabel')}</Typography>
                    </Stack>
                    <OutlinedInput
                        id='keyName'
                        type='string'
                        fullWidth
                        placeholder={t('dialog:apiKey.keyNamePlaceholder')}
                        value={keyName}
                        name='keyName'
