...
const Credentials = () => {
    ...
    const listCredential = () => {
        const dialogProp = {
            title: t('credentials:addNewCredential'),
    ...
    const addNew = (credentialComponent) => {
        const dialogProp = {
            type: 'ADD',
            cancelButtonName: t('credentials:cancel'),
            confirmButtonName: t('credentials:add'),
            credentialComponent
        }
    ...
    const edit = (credential) => {
        const dialogProp = {
            type: 'EDIT',
            cancelButtonName: t('credentials:cancel'),
            confirmButtonName: t('credentials:save'),
            data: credential
        }
        setSpecificCredentialDialogProps(dialogProp)
        setShowSpecificCredentialDialog(true)
    ...
    const share = (credential) => {
        const dialogProps = {
            type: 'EDIT',
            cancelButtonName: t('credentials:cancel'),
            confirmButtonName: t('credentials:share'),
            data: {
                id: credential.id,
                name: credential.name,
                title: t('credentials:shareCredential'),
                itemType: 'credential'
            }
        }
    ...
    const deleteCredential = async (credential) => {
        const confirmPayload = {
            ...
            description: t('credentials:deleteConfirmDescription', { name: credential.name }),
            confirmButtonName: t('credentials:confirmDelete'),
            cancelButtonName: t('credentials:cancel')
        }
        ...
        if (isConfirmed) {
            try {
                ...
                if (deleteResp.data) {
                    enqueueSnackbar({
                        message: t('credentials:credentialDeleted'),
                        options: {
                ...
                enqueueSnackbar({
                    message: t('credentials:deleteError', { message: errorMessage }),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'error',
                        persist: true,
                                ...
                                <Table sx={{ minWidth: 650 }} aria-label='simple table'>
                                    ...
                                    >
                                        <TableRow>
                                            ...
                                            <StyledTableCell>{t('credentials:lastUpdated')}</StyledTableCell>
                                                        ...
                                                        {credential.shared && (
                                                            <>
                                                                <StyledTableCell colSpan={'3'}>
                                                                    {t('credentials:sharedCredential')}
                                                                </StyledTableCell>
                                                            </>
                                                        )}
                                                    </StyledTableRow>
