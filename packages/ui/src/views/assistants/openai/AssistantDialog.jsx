...
const AssistantDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
    ...
    useEffect(() => {
        if (getAssistantObjApi.error) {
            ...
            enqueueSnackbar({
                message: t('assistant.messages.getError', { error: errMsg }),
                options: {
    ...
    const onEditAssistantVectorStoreClick = (vectorStoreObject) => {
        const dialogProp = {
            title: t('assistant.vectorStore.editTitle', { name: vectorStoreObject.name ? vectorStoreObject.name : vectorStoreObject.id }),
            type: 'EDIT',
            cancelButtonName: t('assistant.cancelButton'),
            confirmButtonName: t('assistant.saveButton'),
    ...
    const onAddAssistantVectorStoreClick = () => {
        const dialogProp = {
            title: t('assistant.vectorStore.addTitle'),
            type: 'ADD',
            cancelButtonName: t('assistant.cancelButton'),
            confirmButtonName: t('assistant.addButton'),
            credential: assistantCredential
        }
        setAssistantVectorStoreDialogProps(dialogProp)
        setAssistantVectorStoreDialogOpen(true)
    ...
    const addNewAssistant = async () => {
        ...
        try {
            ...
            if (createResp.data) {
                enqueueSnackbar({
                    message: t('assistant.messages.addSuccess'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('assistant.messages.addError', {
    ...
    const saveAssistant = async () => {
        ...
        try {
            ...
            if (saveResp.data) {
                enqueueSnackbar({
                    message: t('assistant.messages.saveSuccess'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('assistant.messages.saveError', {
                    error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                }),
                options: {
                    key: new Date().getTime() + Math.random(),
    ...
    const onSyncClick = async () => {
        ...
        try {
            ...
            if (getResp.data) {
                ...
                enqueueSnackbar({
                    message: t('assistant.messages.syncSuccess'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('assistant.messages.syncError', {
                    error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                }),
                options: {
                    key: new Date().getTime() + Math.random(),
    ...
    const uploadFormDataToVectorStore = async (formData) => {
        ...
        try {
            ...
            if (uploadResp.data) {
                enqueueSnackbar({
                    message: t('assistant.messages.fileUploadSuccess'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('assistant.messages.fileUploadError', {
                    error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                }),
                options: {
                    key: new Date().getTime() + Math.random(),
    ...
    const onDeleteClick = () => {
        setDeleteDialogProps({
            ...
            description: t('assistant.deleteDescription', { name: assistantName }),
            cancelButtonName: t('assistant.cancelButton')
        })
    ...
    const deleteAssistant = async (isDeleteBoth) => {
        ...
        try {
            ...
            if (delResp.data) {
                enqueueSnackbar({
                    message: t('assistant.messages.deleteSuccess'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('assistant.messages.deleteError', {
                    error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                }),
                options: {
                    key: new Date().getTime() + Math.random(),
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
                    <Box>
                        ...
                        <CredentialInputHandler
                            ...
                            inputParam={{
                                label: t('assistant.connectCredential'),
                                name: 'credential',
                                type: 'credential',
                    ...
                    <Box>
                        ...
                        <Dropdown
                            ...
                            value={assistantModel ?? t('assistant.chooseOption')}
                        />
                    </Box>
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            <Typography variant='overline'>{t('assistant.nameLabel')}</Typography>
                            <TooltipWithParser title={t('assistant.nameTooltip')} />
                        </Stack>
                        <OutlinedInput
                            id='assistantName'
                            type='string'
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            <Typography variant='overline'>{t('assistant.descriptionLabel')}</Typography>
                            <TooltipWithParser title={t('assistant.descriptionTooltip')} />
                        </Stack>
                        <OutlinedInput
                            id='assistantDesc'
                            type='string'
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative' }} direction='row'>
                            <Typography variant='overline'>{t('assistant.iconLabel')}</Typography>
                        </Stack>
                        <div
                            style={{
                                width: 100,
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            <Typography variant='overline'>{t('assistant.instructionsLabel')}</Typography>
                            <TooltipWithParser title={t('assistant.instructionsTooltip')} />
                        </Stack>
                        <OutlinedInput
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            <Typography variant='overline'>{t('assistant.temperatureLabel')}</Typography>
                            <TooltipWithParser title={t('assistant.temperatureTooltip')} />
                        </Stack>
                        <OutlinedInput
                            id='assistantTemp'
                            type='number'
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            <Typography variant='overline'>{t('assistant.topPLabel')}</Typography>
                            <TooltipWithParser title={t('assistant.topPTooltip')} />
                        </Stack>
                        <OutlinedInput
                            id='assistantTopP'
                            type='number'
                    ...
                    {assistantCredential && (
                        <>
                            <Box>
                                <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                                    <Typography variant='overline'>{t('assistant.toolsLabel')}</Typography>
                                    <TooltipWithParser title={t('assistant.toolsTooltip')} />
                                </Stack>
                                <MultiDropdown
                                    ...
                                    options={[
                                        {
                                            label: t('assistant.tools.codeInterpreter'),
                                            name: 'code_interpreter'
                                        },
                                        {
                                            label: t('assistant.tools.fileSearch'),
                                            name: 'file_search'
                                        }
                                    ]}
                                    onSelect={(newValue) => {
                                    ...
                                    value={assistantTools ?? t('assistant.chooseOption')}
                                />
                            </Box>
                            <Box>
                                {assistantTools?.length > 0 && assistantTools.includes('code_interpreter') && (
                                    <Card sx={{ mb: 2, border: '1px solid #e0e0e0', borderRadius: `${customization.borderRadius}px` }}>
                                        <CardContent>
                                            <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                                                <Typography variant='overline'>{t('assistant.codeInterpreter.label')}</Typography>
                                                <TooltipWithParser title={t('assistant.codeInterpreter.tooltip')} />
                                            </Stack>
                                ...
                                {assistantTools?.length > 0 && assistantTools.includes('file_search') && (
                                    <Card sx={{ mb: 2, border: '1px solid #e0e0e0', borderRadius: `${customization.borderRadius}px` }}>
                                        <CardContent>
                                            <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                                                <Typography variant='overline'>{t('assistant.fileSearch.label')}</Typography>
                                                <TooltipWithParser title={t('assistant.fileSearch.tooltip')} />
                                            </Stack>
                                            ...
                                            {!toolResources.file_search || !toolResources.file_search?.vector_store_ids?.length ? (
                                                ...
                                                >
                                                    {t('assistant.vectorStore.addButton')}
                                                </Button>
                                            ) : (
                                                <File
