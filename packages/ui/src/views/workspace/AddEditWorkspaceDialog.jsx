...
const AddEditWorkspaceDialog = ({ show, dialogProps, onCancel, onConfirm }) => {
    ...
    const addNewWorkspace = async () => {
        if (workspaceName === 'Default Workspace' || workspaceName === 'Personal Workspace') {
            enqueueSnackbar({
                message: t('workspace:reservedNameError'),
                options: {
        ...
        try {
            ...
            if (createResp.data) {
                enqueueSnackbar({
                    message: t('workspace:workspaceAdded'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('workspace:addWorkspaceFailed', {
    ...
    const saveWorkspace = async () => {
        try {
            ...
            if (saveResp.data) {
                ...
                enqueueSnackbar({
                    message: t('workspace:workspaceSaved'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('workspace:saveWorkspaceFailed', {
                    error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                }),
                options: {
                    key: new Date().getTime() + Math.random(),
        ...
        >
            <DialogTitle sx={{ fontSize: '1rem' }} id='alert-dialog-title'>
                <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    {dialogProps.type === 'ADD' ? t('workspace:addWorkspace') : t('workspace:editWorkspace')}
                </div>
            </DialogTitle>
            <DialogContent>
                <Box sx={{ p: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'row' }}>
                        <Typography>
                            ...
                            <span style={{ color: 'red' }}>&nbsp;{t('workspace:requiredField')}</span>
                        </Typography>
                ...
                <Box sx={{ p: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'row' }}>
                        <Typography>{t('workspace:descriptionLabel')}</Typography>
                        <div style={{ flexGrow: 1 }}></div>
                    </div>
                    <OutlinedInput
                        size='small'
