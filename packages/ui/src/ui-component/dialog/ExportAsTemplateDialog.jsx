...
const ExportAsTemplateDialog = ({ show, dialogProps, onCancel }) => {
    ...
    const onConfirm = () => {
        if (name.trim() === '') {
            enqueueSnackbar({
                message: t('dialog:exportTemplate.nameRequired'),
                options: {
    ...
    useEffect(() => {
        if (saveCustomTemplateApi.data) {
            enqueueSnackbar({
                message: t('dialog:exportTemplate.saveSuccess'),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'success',
                    persist: true,
    ...
    useEffect(() => {
        if (saveCustomTemplateApi.error) {
            enqueueSnackbar({
                message: t('dialog:exportTemplate.saveError'),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
    ...
    const component = show ? (
        ...
        >
            <DialogTitle sx={{ fontSize: '1rem' }} id='alert-dialog-title'>
                {dialogProps.title || t('dialog:exportTemplate.title')}
            </DialogTitle>
            <DialogContent>
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <Typography sx={{ mb: 1 }}>
                            {t('dialog:exportTemplate.nameLabel')}
                ...
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <Typography sx={{ mb: 1 }}>{t('dialog:exportTemplate.descriptionLabel')}</Typography>
                        <OutlinedInput
                            id={'description'}
                            type={'string'}
                            fullWidth
                ...
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <Typography sx={{ mb: 1 }}>{t('dialog:exportTemplate.badgeLabel')}</Typography>
                        <OutlinedInput
                            id={'badge'}
                            type={'string'}
                            fullWidth
                ...
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <Typography sx={{ mb: 1 }}>{t('dialog:exportTemplate.usecasesLabel')}</Typography>
                        {usecases.length > 0 && (
                        ...
                        <Typography variant='body2' sx={{ fontStyle: 'italic', mt: 1 }} color='text.secondary'>
                            {t('dialog:exportTemplate.usecasesHint')}
                        </Typography>
                    </div>
                </Box>
            </DialogContent>
            <DialogActions>
                ...
                <StyledButton disabled={dialogProps.disabled} variant='contained' onClick={onConfirm}>
                    {dialogProps.confirmButtonName || t('dialog:exportTemplate.saveButton')}
                </StyledButton>
            </DialogActions>
        </Dialog>
    ) : null
