...
const ChatFeedback = ({ dialogProps }) => {
    ...
    const onSave = async () => {
        try {
            ...
            if (saveResp.data) {
                enqueueSnackbar({
                    message: t('chatFeedback.saveSuccess'),
                    options: {
            ...
            enqueueSnackbar({
                message: `${t('chatFeedback.saveError')}: ${
    ...
    return (
        <>
            <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                <SwitchInput label={t('chatFeedback.enableLabel')} onChange={handleChange} value={chatFeedbackStatus} />
            </Box>
            <StyledButton style={{ marginBottom: 10, marginTop: 10 }} variant='contained' onClick={onSave}>
                {t('chatFeedback.saveButton')}
            </StyledButton>
        </>
    )
}
